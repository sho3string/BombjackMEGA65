...
TEHKAN_SET2_FILES = [
    "09_j01b.bin", "10_l01b.bin","11_m01b.bin","12_n01b.bin", "13_r01b.bin", 
    "01_h03t.bin",
    "03_e08t.bin","04_h08t.bin","05_k08t.bin",
    "06_l08t.bin","07_n08t.bin","08_r08t.bin",
    "16_m07b.bin","15_l07b.bin","14_j07b.bin",
...
TEHKAN_SET2_CHK = {
    ...
    "11_m01b.bin": "43bae56494ac0202aaa8f1ed5c1ed1bff775b2b8",
    "12_n01b.bin": "8b3c49e21ea4952cae7042890d1be2115f7d6fda",
    "13_r01b.bin": "d243eb1249e885aa75fc910fce6e7744770d6e82",
    "01_h03t.bin": "318face9f7a7ab6c7eeac773995040425e780aaf",
    "03_e08t.bin": "94ef52ef47b4399a03528fe3efeac9c1d6983446",
    "04_h08t.bin": "e29ba193f21aa898499187603b25d2e226a07c7b",
    "05_k08t.bin": "a66808ef2d62fca2854396898b86bac9be5f17a3",
...
def main():
    ...
    print("Bombjack for MEGA65: ROM Installer")
    print("================================\n")
    if len(sys.argv) != 3:
    ...
    if len(sys.argv) > 1:
        ...
        elif fileName == "bombjack2.zip":     # Galaga Midway set 1
            EXPECTED_FILES=TEHKAN_SET2_FILES
            EXPECTED_CHKSM=TEHKAN_SET2_CHK
        else:
            print ("No match found for",sys.argv[1],"\n")
            print ("Suitable roms are bombjack.zip, bombjack2.zip, bombjack\n")
            return
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            with zipfile.ZipFile(rom_zip_path, 'r') as zip_ref:
                ...
                if fileName == "bombjack.zip" or fileName == "bombjack2.zip":
                    ...
                    with open(os.path.join(output_folder, EXPECTED_FILES[6]), "wb") as rom:
                            for part in [EXPECTED_FILES[6], EXPECTED_FILES[6]]: 
                                print(f"Appending {part} to ", EXPECTED_FILES[6])
                                with open(os.path.join(temp_dir, part), "rb") as f:
                                    rom.write(f.read())  
                   
                    with open(os.path.join(output_folder, EXPECTED_FILES[7]), "wb") as rom:
                            for part in [EXPECTED_FILES[7], EXPECTED_FILES[7]]: 
                                print(f"Appending {part} to ", EXPECTED_FILES[7])
                                with open(os.path.join(temp_dir, part), "rb") as f:
                                    rom.write(f.read())  
                    
                    with open(os.path.join(output_folder, EXPECTED_FILES[8]), "wb") as rom:
                            for part in [EXPECTED_FILES[8], EXPECTED_FILES[8]]: 
                                print(f"Appending {part} to ",EXPECTED_FILES[8])
                                with open(os.path.join(temp_dir, part), "rb") as f:
                                    rom.write(f.read())  
                                    
                                    
                    for filename in [EXPECTED_FILES[9],EXPECTED_FILES[10],EXPECTED_FILES[11],EXPECTED_FILES[12],EXPECTED_FILES[13],EXPECTED_FILES[14]]:
                        print(f"Copying {filename} to output folder")
                        shutil.copy(os.path.join(temp_dir, filename), output_folder)
                        
                    with open(os.path.join(output_folder, EXPECTED_FILES[15]), "wb") as rom:
                            for part in [EXPECTED_FILES[15], EXPECTED_FILES[15]]: 
                                print(f"Appending {part} to ",EXPECTED_FILES[15])
                                with open(os.path.join(temp_dir, part), "rb") as f:
                                    rom.write(f.read())  
                                    
                    #Create the bjcfg file
                    print("\nCreating bjcfg file")                
                    output_file_path = os.path.join(output_folder, "bjcfg")
                    with open(output_file_path, "wb") as binary_file:
                        binary_file.write(bytes([0xff]) * 56)
